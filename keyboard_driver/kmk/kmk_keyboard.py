...
class KMKKeyboard:
    ...
    def _process_resume_buffer(self):
        """Resume the processing of buffered, delayed, deferred, etc. key
    ...
    def _init_coord_mapping(self) -> None:
        """Attempt to sanely guess a coord_mapping if one is not provided. No-
        op if `kmk.extensions.split.Split` is used, it provides equivalent
        functionality in `on_bootup`

        To save RAM on boards that don't use Split, we don't import
        Split and do an isinstance check, but instead do string
        detection
        """
        if any(
    ...
    def during_bootup(self) -> None:
        ...
        if debug.enabled:
            debug(
                "extensions=", [_.__class__.__name__ for _ in self.extensions]
            )
