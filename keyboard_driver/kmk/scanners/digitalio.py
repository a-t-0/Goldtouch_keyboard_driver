...
class MatrixScanner(Scanner):
    def __init__(
        ...
        if self.diode_orientation == DiodeOrientation.COLUMNS:
            ...
            raise ValueError(
                f"Invalid DiodeOrientation: {self.diode_orienttaion}"
            )
    ...
    def scan_for_changes(self):
        """Poll the matrix for changes and return either None (if nothing
        updated) or a bytearray (reused in later runs so copy this if you need
        the raw array itself for some crazy reason) consisting of (row, col,
        pressed) which are (int, int, bool)"""
        ba_idx = 0
        any_changed = False
