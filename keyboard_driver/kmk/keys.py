...
def maybe_make_shifted_key(candidate: str) -> Optional[Key]:
    ...
    for code, names in codes:
        if candidate in names:
            return make_key(
                code=code, names=names, has_modifiers={KC.LSFT.code}
            )
...
class Key:
    ...
    def clone(self) -> Key:
        """Return a shallow clone of the current key without any pre/post
        press/release handlers attached.

        Almost exclusively useful for creating non-colliding keys to use
        such handlers.
        """
    ...
    def before_press_handler(
        self, fn: Callable[[Key, Keyboard, ...], bool]
    ) -> None:
        """
