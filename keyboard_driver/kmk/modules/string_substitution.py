...
class Character:
    """Helper class for making a left-shifted key identical to a right-shifted
    key."""
...
class Phrase:
    """Manages a collection of characters and keeps an index of them so that
    potential matches can be tracked."""
    ...
    def character_is_at_current_index(self, character) -> bool:
        """Returns True if the given character is the next character in the
        phrase."""
        ...
        if is_pressed:
            ...
            for rule in self._rules:
                if rule.to_substitute.character_is_at_current_index(character):
                    ...
                    if rule.to_substitute.character_is_at_current_index(
                        character
                    ):
                        rule.to_substitute.next_character()
    ...
    def before_hid_send(self, keyboard):
        ...
        if self._state == State.SENDING:
            ...
            if not substitution.index_at_end():
                keyboard.tap_key(
                    substitution.get_character_at_current_index().key_code
                )
