...
class DynamicSequences(Module):
    ...
    def record_frame(self, keys_pressed):
        if (
            self.current_slot.sequence_data[self.index].keys_pressed
            != keys_pressed
        ):
            self.index += 1
            ...
            if self.use_recorded_speed:
                self.current_slot.sequence_data.append(
                    SequenceFrame(
                        keys_pressed.copy(),
                        ticks_diff(ticks_ms(), self.start_time),
                    )
                )
    ...
    def stop_recording(self):
        ...
        self.current_slot.sequence_data.append(
            SequenceFrame(
                set(), self.current_slot.sequence_data[-1].timestamp + 20
            )
        )
    ...
    def play_frame(self, keyboard):
        ...
        if not check_deadline(
            ...
            if self.index:
                prev = self.current_slot.sequence_data[
                    self.index - 1
                ].keys_pressed
            ...
            self.index += 1
            if self.index >= len(
                self.current_slot.sequence_data
            ):  # Reached the end
        ...
        for key in keyboard.keys_pressed.difference(self.last_config_frame):
            if key.code in _numbers:
                ...
                if self.status == SequenceStatus.SET_REPEPITIONS:
                    ...
                    self.current_slot.interval = (
                        self.current_slot.interval * 10 + digit
                    )
