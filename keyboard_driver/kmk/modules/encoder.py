    ...
    def update_state(self):
        ...
        if new_state != self._state:
            ...
            if (
                ...
                and self._state[0] != self._state[1]
            ):
...
class GPIOEncoder(BaseEncoder):
    def __init__(
        ...
        if pin_button:
            self.pin_button = EncoderPin(
                pin_button, button_type=True, pull=button_pull
            )
        else:
        ...
        if self.pins and self.map:
            for idx, pins in enumerate(self.pins):
                try:
                    ...
                    new_encoder.on_move_do = (
                        lambda x, bound_idx=idx: self.on_move_do(
