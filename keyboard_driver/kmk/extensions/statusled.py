...
class statusLED(Extension):
    ...
    def _layer_indicator(self, layer_active, *args, **kwargs):
        ...
        indicator. And when there are more layers than leds it wraps
        around to the first led again. (Also works for a single led,
        which just lights when any layer is active)
        """

        if self._layer_last != layer_active:
            led_last = (
                0 if self._layer_last == 0 else 1 + (self._layer_last - 1) % 3
            )
            if layer_active > 0:
                led_active = (
                    0 if layer_active == 0 else 1 + (layer_active - 1) % 3
                )
