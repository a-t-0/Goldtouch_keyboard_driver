...
def compile_unicode_string_sequences(string_table):
    """Destructively convert ("compile") unicode strings into key sequences.

    This will, for RAM saving reasons, empty the input dictionary and
...
def unicode_string_sequence(unistring):
    """Allows sending things like (╯°□°）╯︵ ┻━┻ directly, without manual
    conversion to Unicode codepoints."""
    return unicode_codepoint_sequence(
        [hex(get_wide_ordinal(s))[2:] for s in unistring]
    )
...
def unicode_codepoint_sequence(codepoints):
    kc_seqs = (
        generate_codepoint_keysym_seq(codepoint) for codepoint in codepoints
    )
    ...
    def _unicode_sequence(key, keyboard, *args, **kwargs):
        if keyboard.unicode_mode == UnicodeMode.IBUS:
            keyboard.process_key(
                simple_key_sequence(
                    _ibus_unicode_sequence(kc_macros, keyboard)
                ),
                True,
            )
            ...
            keyboard.process_key(
                simple_key_sequence(
                    _ralt_unicode_sequence(kc_macros, keyboard)
                ),
                True,
            )
            ...
            keyboard.process_key(
                simple_key_sequence(
                    _winc_unicode_sequence(kc_macros, keyboard)
                ),
                True,
            )
